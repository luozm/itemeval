...
4. **Live smoke test** (costs ~$0): `make test-live` — a real two-model
   ...
   before CC pushes a `feat/*`, `fix/*`, or `chore/*` branch (an inspect bump is
   a `chore/*`), self-skips without a key (the test loads `.env`), and never
   runs in CI (the `live` marker is deselected by `make test`/`check`).
