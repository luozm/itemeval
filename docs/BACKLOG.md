...
### Mid-run liveness tracker (live heartbeat + live ETA)
**Key:** `live-tracker`

**Motivation.** A running `generate`/`grade` stage can go dark. inspect's rich
progress bars show on a TTY, but a backgrounded run, a non-TTY pipe, or any
`--json` run silences them (`--json` forces `display=none`) — so a long paid run
emits nothing until it returns (the `--json`-dark bug in KNOWN-ISSUES). Operators
then guess liveness from a log file's mtime (a false signal), and the pre-flight
ETA is a coarse prior that never updates with real throughput. A flaky-endpoint
run you can't see is the run you panic-kill.

**Design sketch.** One inspect `SampleEnd`/`SampleStart` hook (a published
extension point — wrap, don't fork) in a new extension-tier module, firing
in-process as each sample completes, emitting a **throttled plain-text heartbeat to
stderr** — independent of `--json`, so stdout stays pure JSON and a captured-stderr
caller still sees life (relay-safe per UX-PATTERNS Law 8/the relay rule):

    # sketch (stderr, throttled ~every 10s)
    [itemeval] generate · exp a7b3c9d2/a1 · 142/400 (35%) · 11/min · ~3m left · 2 errors · 8 in-flight

The line carries run state in itemeval terms (cells done of the run's expected,
the experiment_id/attempt from `recovery-run-identity`, errors, in-flight) and a
**live ETA** from observed throughput (refining the rough pre-flight prior). It is
gated to when inspect's live display is silenced (`resolve_display(...) == "none"`,
which `--json`/`--display none` produce), so it never fights the rich bars and
never fires in a notebook. The hook is process-global, so it acts only while
itemeval's own eval runs (a module-level run context the orchestrator sets around
`run_condition_evals`) and is throttled by time/count (the callback is awaited
inside `eval()` — it must stay cheap). **No `.eval` read, no parquet write — the
heartbeat is a counter.**

Cross-terminal live `status` needs nothing new: `recoverable-harvest`'s
read-triggered projection already reads an in-flight `.eval` from disk (inspect
flushes samples incrementally), so `itemeval status` in another terminal already
reports a running stage's completed cells; `watch itemeval status` covers polling.

**Scope decision (this supersedes the run-UX tracker's "C owns S's deferred L3").**
The `on_sample_end` hook ships, but drives the **heartbeat, not a parquet flush**.
A live store by per-sample parquet upsert is redundant (inspect's incremental
`.eval` + the projection above already make `status` see mid-run rows) and harmful
(parquet is whole-file rewrite → O(N²); the hook is awaited inside the eval → a
write per sample slows the run). "Truly live store" = inspect's WAL + the
read-triggered projection, read on demand.

**Implementation notes.** New extension module `_tracker.py`: the `@hooks` class +
a module-level run context (active flag, expected total, stage, experiment/attempt,
counters, start time, throttle). It **pre-latches inspect's `init_hooks()` before
registering**, because registering any hook makes `init_hooks` `rich.print` a
"hooks enabled" banner to stdout (would corrupt `--json`); `enabled()` returns the
active flag so the hook is a no-op outside itemeval runs. `generate/_run.py`
imports `_tracker` at top (registers the hook before the first `eval()`) and
`run_condition_evals` sets/clears the run context (computing `total` from the
tasks) and decides heartbeat-on from the resolved display. `cli.py` (`_run_stage`)
routes the pre-flight ETA line to stderr under `--json` (the KNOWN-ISSUE fix).
`status`/export untouched (they already project). inspect imports confined to the
extension module + orchestrator. ~120 lines + tests (mockllm drives the hook
deterministically; a guard test asserts `--json` stdout stays one clean JSON doc).
**No new config knob, exit code, JSON key, gate, or hint.** Closes the `--json`-dark
KNOWN-ISSUE in the same change (remove that section; the feature's CHANGELOG entry
notes the closure). Relaxes the Agent-Guide's "`--json` goes dark" carve-out back
to "`--json` everywhere is safe (liveness rides stderr)."

**Open questions.**
- *Partial-`.eval` metadata timing* (the one technical risk): does a mid-run
  partial `.eval` carry the eval `metadata` (experiment_id/attempt/condition_id)
  that `harvest` keys on, or only the finish-written header? If early
  (`start.json`), cross-terminal live `status` works for free; if late, the
  heartbeat (which reads no `.eval`) is the sole mid-run signal until finish —
  robust either way, but it decides how much mid-run `status` shows. Verify against
  the pinned inspect.
- Heartbeat granularity: run-level counts + a per-experiment label (lean), with a
  per-condition tail only on errors.
- Whether to add `status --watch` (a redraw loop) or rely on `watch itemeval
  status` (lean: defer the flag).
