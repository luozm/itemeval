...
| ✅ `split-head-below-min` | split enabled, shared head below the provider's minimum (estimate-time, fires on estimate/generate/grade) | `hint: split_rubric is on but 7/40 judge heads are under anthropic/claude-haiku-4-5's ~4096-token cache minimum (chars/4 estimate) — those groups will not engage the provider cache — learn more: Cost-Savings#two-gotchas` | Cost-Savings |
| ✅ `anthropic-openrouter-no-split` | Anthropic model via OpenRouter runs monolithic with caching otherwise active (known zero discount, verified live 2026-06-12; estimate-time, fires on estimate/generate/grade; the estimator projects no discount for these conditions) | `hint: openrouter/anthropic/claude-haiku-4.5 won't get cache discounts via OpenRouter without split_prompt — monolithic prompts get no cache marker (the projection shows full price) — learn more: Cost-Savings#prompt-packaging` | Cost-Savings |
| ✅ `openrouter-unpinned-cache` | `openrouter/anthropic/*` runs cached without `provider_routing` (routing may land on a marker-ignoring upstream); suppressed when the model is routed to its native batch API (`prefer_native_batch`), as the call never goes through OpenRouter | `hint: openrouter/anthropic/claude-haiku-4.5 ran cached via OpenRouter without solvers/graders provider_routing — routing may land on an upstream that ignores cache markers (silent full price) — learn more: Cost-Savings#openrouter-or-direct` | Cost-Savings |
...
| ✅ `empty-materialized-rubrics` | a materializing rubric returned no text for N items (graded against a blank `{rubric}`) | `hint: 3 materialized rubric(s) came back empty — the materializer (openrouter/openai/gpt-5.4) produced no text, so those items were graded against a blank rubric — learn more: Error-Handling#empty-materialized-rubrics` | Error-Handling |
| ✅ `truncated-completions` | N non-empty completions stopped at a length cap (`max_tokens`/`model_length`) — graded as finished though cut short (fires on `generate`) | `hint: 21 completion(s) stopped at a length cap (max_tokens/model_length) with non-empty text — they are scored as finished answers though they were cut short; raise solvers.max_tokens or filter truncated rows — learn more: Error-Handling#truncation` | Error-Handling |
| ✅ `reroute-residue` | N cells still soft-failed after the `solvers.max_reroutes` cap (fires on `generate`) | `hint: 2 cell(s) still soft-failed after 3 reroute(s) (excluded: GMICloud, Phala) — they carry an honest soft-failure row, not a score; exclude them in analysis, pin solvers.provider_routing, or (single-provider models) substitute the model — learn more: Error-Handling#soft-failures-and-reroute` | Error-Handling |
...
| ✅ `native-batch-available` | a batch run has `openrouter/*` models with an eligible native batch endpoint (key present) but `budget.prefer_native_batch` is off — the ~50% batch discount is left unused (estimate-time) | `hint: 1 model(s) could route to their native batch API (anthropic) to save ~$4.20 — set budget.prefer_native_batch (the sampled id stays the model's identity) — learn more: Cost-Savings#native-batch-routing` | Cost-Savings |
...
| Model sample pin write | `model_locks.json` (study dir, but **decides future runs**) | **compliant** — `models:` line on first draw; roster drift warned, never blocks; an additive spec field never bricks (normalized compare); a genuinely-changed spec hard-fails the draw/write path with a change briefing (read-only commands warn `spec_drift` and inspect the pinned panel); `itemeval rebless` records the changed spec keeping the panel (no re-draw, announced); `model_sample` in JSON / manifest / study card | `models: sampled N of M (seed S…) from <source> — pinned in model_locks.json` (first draw); reuse/drift/`pinned panel`/`re-blessed` line on later runs; `re-blessed: model_locks.json now records the current solvers.sample spec — N models kept` |
...
| Endpoint-window lookup | network → OpenRouter `/models/:slug/endpoints` (roster models only, on `generate`); writes `~/.cache/itemeval/endpoints.json` | **compliant** — run-summary line + `endpoint_windows_fetched`/`endpoint_windows_reused`/`endpoint_cache_dir` on results | `endpoint windows: fetched N from OpenRouter — cache dir: <path>` (or `M reused`) |
...
| Serving-endpoint routing (native batch) | network → a *different* provider account than the sampled id names (`budget.prefer_native_batch`) | **compliant** — `native batch routing:` line on estimate/generate/grade when active; `routed_models` on run results, `execution_model`/`routed` per manifest `endpoints_effective` entry; ledger `provider` = the billing (native) provider. A provider whose native batch is broken upstream (`NATIVE_BATCH_BROKEN`, currently google — inspect_ai's `GoogleBatcher` array-vs-`{parts}` bug) is **never** routed to native batch and a directly-named `google/*` model runs interactively even under a batch plan; the fallback is announced (`warnings[]`) and the 50% batch discount is withheld from its cost/estimate/`batch` provenance | `native batch routing: N model(s) → native API (anthropic) — sampled ids stay the scientific identity; native id recorded as execution_model` · `native batch unavailable for N model(s) — ran interactively (inspect_ai serialization bug; no batch discount): google/…` |
...
| Crash-recovery harvest (a read/resume command writes recovered rows) | solutions/gradings parquet (study dir) — projected from a prior run's on-disk `.eval` | **compliant** — `status`/`export`/`generate`/`grade` auto-harvest a crashed run's `.eval` into the store first and announce it when rows were recovered; `--no-harvest` opts out; `harvested` object on each command's `--json`; explicit `itemeval harvest`. In-dir write (like row replacement above) but announced because it *extends a read command's contract* | `recovered N solutions + M gradings from K interrupted run log(s) into the store (logs/…)` (printed only when rows were recovered) |
| Pre-flight model probe | network → provider (one ~1-token call per distinct model; sub-cent spend) | **compliant** — `itemeval preflight` fires the probes and announces them in one line; `models[]` + `ok`/`dead`/`unverified` on `--json`. A *deliberately-invoked* command (like `estimate`), so invoking it is the consent to its spend — no gate, no auto-run inside generate/grade | `preflight: probed N distinct model(s) over the provider network (~1 token each) — X ok · Y dead · Z unverified` |
