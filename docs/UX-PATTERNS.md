...
| Model sample pin write | `model_locks.json` (study dir, but **decides future runs**) | **compliant** — `models:` line on first draw; roster drift warned, never blocks; `model_sample` in JSON / manifest / study card | `models: sampled N of M (seed S…) from <source> — pinned in model_locks.json` (first draw); reuse/drift line on later runs |
