...
| Model sample pin write | `model_locks.json` (study dir, but **decides future runs**) | **compliant** — `models:` line on first draw; roster drift warned, never blocks; an additive spec field never bricks (normalized compare); a genuinely-changed spec hard-fails the draw/write path but read-only commands warn (`spec_drift`) and inspect the pinned panel; `model_sample` in JSON / manifest / study card | `models: sampled N of M (seed S…) from <source> — pinned in model_locks.json` (first draw); reuse/drift/`pinned panel` line on later runs |
