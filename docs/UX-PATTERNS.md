...
| ✅ `openrouter-unpinned-cache` | `openrouter/anthropic/*` runs cached without `provider_routing` (routing may land on a marker-ignoring upstream) | `hint: openrouter/anthropic/claude-haiku-4.5 ran cached via OpenRouter without solvers/graders provider_routing — routing may land on an upstream that ignores cache markers (silent full price) — learn more: Cost-Savings#openrouter-or-direct` | Cost-Savings |
