...
| Serving-endpoint routing (native batch) | network → a *different* provider account than the sampled id names (`budget.prefer_native_batch`) | **compliant** — `native batch routing:` line on estimate/generate/grade when active; `routed_models` on run results, `execution_model`/`routed` per manifest `endpoints_effective` entry; ledger `provider` = the billing (native) provider | `native batch routing: N model(s) → native API (anthropic) — sampled ids stay the scientific identity; native id recorded as execution_model` |
