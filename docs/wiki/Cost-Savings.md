...
hint (not raised when `prefer_native_batch` routes the model to its native
batch API — the call no longer goes through OpenRouter). And you can verify the
pin held after any run: the run's manifest
