...
solvers:
  ...
  attempt_timeout: null        # optional; seconds. Per-attempt request timeout passed to inspect: a stalled attempt is abandoned and retried (via OpenRouter, possibly onto a healthier upstream) up to max_retries attempts, THEN the cell errors. default null = no per-attempt bound. Leave unset for batch runs — it would bound the long batch poll too. See Error-Handling#stalled-requests-attempt_timeout. Execution knob; never changes condition ids.
  max_retries: null            # optional; int. Max attempts for a model call before it gives up and the cell errors (inspect GenerateConfig.max_retries). Bounds both timeout and transient-HTTP retries. default null = inspect's behavior, EXCEPT when attempt_timeout is set, where it defaults to 2 so a stalled call can't loop forever. Execution knob; never changes condition ids.
  retry_on_error: null         # optional; int >= 0. SAMPLE-level retry (inspect eval(retry_on_error=...)): a sample that errors (an attempt-timeout hole, or a transient error) is re-run up to N times — a SEPARATE layer from max_retries (which retries within one model call). default null = the built-in 1; 0 = single attempt, so a slow cell holes after one attempt_timeout instead of two (a deliberately fail-fast "fast pass" — fill holes later). Generate stage only. Execution knob; never changes condition ids.
