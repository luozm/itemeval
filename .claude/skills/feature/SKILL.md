...
7. **Same-change rule** (same commit as the behavior): add a `[Unreleased]` entry to @CHANGELOG.md with a `Closes: $ARGUMENTS` trailer; **remove** the shipped section from @docs/BACKLOG.md; if @ROADMAP.md named `$ARGUMENTS` as a future candidate, move it to the `**Already landed**` line (a shipped key is no longer a backlog candidate — `tests/test_docs_consistency.py` fails otherwise); update the wiki and UX-PATTERNS rows if the user-facing surface changed.
