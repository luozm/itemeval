...
7. **Archive the plan** when the feature is fully shipped: stamp `docs/plans/$ARGUMENTS.md` as `IMPLEMENTED <today's date>`, `git mv` it to `docs/plans/archive/`, and fix inbound links (grep the filename) — the plan lifecycle in @docs/plans/TEMPLATE.md. Leaving it active is the most common drift after a ship.
8. **Green:** `make check`. If you changed the public API or CLI surface, expect `tests/test_public_api_snapshot.py` to go red — update the golden set deliberately, in the same change.
