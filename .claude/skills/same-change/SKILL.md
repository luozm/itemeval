...
2. **BACKLOG disjointness.** Does any key that now appears in a CHANGELOG `Closes:` still have a `**Key:**` section in @docs/BACKLOG.md? A shipped feature must have *left* the backlog. This is the invariant `tests/test_docs_consistency.py` enforces — run `make docs-check` to confirm machine-side.
