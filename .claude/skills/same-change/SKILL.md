...
7. **Study-facing schema.** If the change touches a persisted study-facing schema — a parquet store, `model_locks.json` / `dataset_locks.json`, `manifests/`, or the condition-id hash inputs (see @DEVELOPMENT.md "Study-facing schema evolution") — is it either *additive-with-default* **and** covered by an older-fixture guard test, **or** *non-additive* **and** carrying a `Study migration` note in the CHANGELOG entry plus either a safe reconcile path **or** (pre-1.0 only) a *result-preserving clean-break tip* per @DEVELOPMENT.md "Pre-1.0 carve-out" (delete-and-re-run that replays identical results)? Clearing a pin (`model_locks.json` / `dataset_locks.json`) to force a re-draw is never an acceptable tip — it silently changes the panel.
