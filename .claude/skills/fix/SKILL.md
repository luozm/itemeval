...
5. **CHANGELOG (+ tracker).** Add a `[Unreleased]` → `Fixed` entry to @CHANGELOG.md in user-facing wording (bugs need no backlog key). If this bug was tracked in @docs/KNOWN-ISSUES.md, remove its entry in the same commit.
