...
6. **Commit the entry** once I confirm — a `docs: record <key> candidate in BACKLOG` commit on `main`. That committed entry **is** the brainstorm deliverable: it puts the key on `main`, where `/feature <key>` expects to find it (and where `/feature` commits the plan before branching). Leaving it uncommitted is what later forces the entry onto a feature branch by accident.
